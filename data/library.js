...
/* ======================= GEOPOLITICS x TECHNOLOGY ======================= */
{
  ...
  metrics: [
    ...
    { v: "41", l: "Notified bodies active under Reg (EU) 2023/1230, Commission register re-read 2026-09-22, was 40 on 09-18" },
    { v: "152", l: "Notified bodies active under Directive 2006/42/EC, the instrument 2023/1230 replaces, was 153 on 09-18" },
    { v: "27.0%", l: "New regime as a share of the outgoing one, 120 days before it applies, was 26.1% on 09-18" }
  ],
  body: [
    ["The obligation", "Annex I Part A item 5 explicitly names \"safety components with fully or partially self-evolving behaviour using machine learning approaches ensuring safety functions\" as a high-risk category. That removes the self-certification option and mandates third-party Notified Body assessment through Module B (EU type-examination), Module H (full quality assurance) or Module G (unit verification). TÜV SÜD was the first NB designated under the new Regulation, September 2024. NB capacity is finite, and since 2026-09-18 it is also counted and now tracked as a diff. The Commission's Single Market Compliance Space register, filtered on notification status Active, returned 40 bodies under Regulation (EU) 2023/1230 against 153 under Directive 2006/42/EC on 2026-09-18, and 41 against 152 when re-read on 2026-09-22. Both lines moved in four days and the ratio went from 26.1% to 27.0%, 120 days before the Regulation applies. Intertek Deutschland GmbH, NB 0905, appears under the Regulation in the 09-22 reading; this card previously had it accredited on 5 December 2025 with designation not confirmed, so that gap is closed. The body responsible for 40 becoming 41 cannot be named, because the 09-18 reading stored a count and not a list; the 09-22 names are now in reports/knowledge-state.md so later runs diff names. The register renders only in a browser; it returns an application shell to HTTP clients, which is why this count was recorded as not retrieved on four previous runs."],
...
/* ================================ THESES ================================ */
{
  ...
  metrics: [
    ...
    { v: "41", l: "Notified bodies active under Reg (EU) 2023/1230, against 152 under the Directive it replaces on 2027-01-20, re-read 2026-09-22" },
    { v: "55%", l: "Self-modifications that improved the triggering failure while degrading a previously working case, 211 of 383 (arXiv 2609.24130)" },
    { v: "$50M", l: "Total funding at Raindrop, which now sells replay of production traffic against a proposed change" }
  ],
  body: [
    ...
    ["Both directions again, 2026-09-22, and the kill test nearly resolved", "Supporting: the Self-Healing Harness (arXiv 2609.24130, 21 September) wraps admission control around an agent that edits its own operating instructions, admitting a change only \"after measured improvement on the triggering failure without regression beyond a fixed margin on protected cases\". Its gate \"rejected 383 replay-decided proposals. Of these, 211 (55%) improved their triggering failure while degrading a case that previously worked.\" That is the first measured base rate this board holds for the surviving claim's core mechanism, a local improvement concealing a regression, and 55% is not a tail case. The bound: it gates a text context rather than model weights, and the team shipping the change chooses the protected cases. Weakening, at equal prominence and arguably the stronger of the two: Raindrop announced a Series A led by CRV taking total funding to $50 million, with Lightspeed, Y Combinator and named OpenAI, Anthropic and Thinking Machines researchers participating and no valuation stated, and shipped Simulations, which \"replay real production traffic and existing test cases against a proposed change to an agent harness, then apply Raindrop's anomaly detection to the results\" so that engineers \"see what their change will change\". The kill test asks for one buyer paying for paired non-regression testing. These buyers are paying, which is new, but they are buying a vendor's tool rather than a third party's verdict, so the commons argument has mutated into a SaaS argument rather than being answered. Raindrop is not a new entrant either: it is already in the board's own YC census as a Winter 2024 company, one-liner \"Sentry for AI Agents\". THE POINT THAT NOW DECIDES THIS THESIS, and it is a decision rather than a retrieval: every replay system built so far, Chronicle on 17 September and these two on 17 and 21 September, replays a digital boundary. Nothing has replayed physical contact. Restated as non-replayability of the physical world rather than non-regression in general, the thesis is untouched by any of them; left as non-regression in general, it is being commoditised in public. Also on self-issued certificates, supporting: arXiv 2609.23478 (20 September) shows the \"label-free certificate\" used to validate latent action models does not establish what it claims, since \"a trained but unconstrained counterpart already achieves 83-97% of the reduction relative to an untrained anchor\". And arXiv 2609.23957 (21 September) separates reproducibility from validity on the same artefacts: enforced checks \"raised fresh-run reproduction from one of eight to eight of eight but could not detectably improve conclusion validity, because fifteen of sixteen agents selected the same audit-excluded entry\"."],
...
{
  id: "astra-embodiment-transfer",
  axis: "thesis",
  kind: "report",
  title: "What does a new body cost, in demonstrations?",
  dek: "The submission plan for the 24 Oct Astra hackathon: train 26 policies from scratch across two robot morphologies in one room and put a price on the embodiment gap.",
  status: "current",
  date: "2026-09-20",
  owner: "anshu",
  open: "reports/astra-embodiment-transfer-2026-09-20.md",
  openLabel: "Open plan",
  tags: ["hackathon", "embodiment gap", "cross-embodiment", "end-to-end training", "24 Oct 2026"],
  metrics: [
    { v: "26", l: "Policies trained from scratch on the day — 3 transfer treatments x 4 target-data budgets x 2 directions, plus 2 within-embodiment oracles" },
    { v: "demos-to-threshold", l: "The unit the embodiment gap gets priced in. The field reports transfer success rates; nobody reports transfer prices." },
    { v: "2 bodies, 1 table", l: "OpenArm with a parallel gripper against the Wuji Hand 2, same task, same seeded placements, same lighting, same day" },
    { v: "k = 0, 5, 15, 40", l: "Target-embodiment demonstration budgets swept, which is what turns a yes/no transfer claim into a curve" }
  ],
  body: [
    ["Why it is on the board", "Registered under the standing rule that robotics, Stanford and startup artefacts get a space here (gbrain concepts/portal-artefact-rule). It is a plan, not a result — carded now so the 24 October date is visible from the board and so the measured numbers can be written back onto this same card in late October."],
    ["The pivot", "This supersedes the 18 September certificate-desk plan AS THE THING TO SUBMIT. Three problems with that one: its best version depended on other teams opting in, so it was outside its own control; it reads as criticism of the floor however carefully the tone is managed; and it trained nothing, which is the wrong shape for a day whose scarce resource is robots and whose tracks are dexterous manipulation, teleoperation and whole-body control. The measurement rig is not wasted — tools/rig/stats.py, the SPRT and the bootstrap, committed 4e17a4f — it stops being the product and becomes the reason twenty-six policies get through one arm before 20:00."],
    ["The question", "'Policies do not transfer across robots' is a qualitative claim nobody has to pay for. Make it a currency: hold the task fixed, train on source body A, and ask how many demonstrations on target body B are needed before the policy clears a fixed success threshold on B. That number is what a deployment engineer actually budgets against — the question in the field is never whether it transfers, it is what the next arm will cost."],
    ["The three treatments", "JOINT: raw joint position commands, embodiment-specific by construction, the null. EEF: end-effector delta pose plus a scalar gripper aperture, retargeted by IK — the cheap abstraction that assumes bodies differ in kinematics and not in task. SHARED: one trunk, a per-embodiment action head and a learned embodiment token, the CrossFormer/Octo recipe shrunk to hackathon size. Every model is trained FROM SCRATCH with no pretrained VLA, and that is not asceticism — any open pretraining corpus already contains both body classes, so fine-tuning would confound 'transfers' with 'was memorised'."],
    ["Why this event and not a lab", "The experiment needs two robot morphologies with matched teleoperation on one table, same objects, same lighting, same day. A university lab has one arm. Simulation cannot answer it, because the embodiment gap IS the part that lives in real contact dynamics, cable slop and gripper compliance. A room holding an OpenArm, a Wuji Hand 2, ALOHA leader arms and an M1 is a cross-embodiment natural experiment that is expensive to assemble and is being given away for a Saturday."],
    ["Where the day actually breaks", "Not training — twenty-six small ACT-class runs at 10-25 minutes each, four in parallel, close in about three hours. The bottleneck is ARM TIME for evaluation: twenty-six policies at twenty trials each is four hours that do not exist. Two mechanisms make it fit. The SPRT stops each policy as soon as its declared threshold is decided, typically at eight to fourteen trials. And the grid is allocated adaptively — run the k=0 and k=40 corners for all three treatments first, then spend what is left only on interiors whose corners came out separated."],
    ["All three answers are interesting", "If EEF is indistinguishable from JOINT, the cheap abstraction buys nothing and a large amount of current practice is retargeting into a shared action space and calling the embodiment problem addressed. If SHARED is flat down to k=0, the gap is representational and small models close it — which is an argument that the world-model route buys something real rather than buying scale, the position already on record in the brain. If nothing clears the threshold below k=40, a new body costs a fresh dataset regardless of strategy, and data-collection infrastructure beats representation research."],
    ["The dominant risk is floor effects", "If the within-embodiment oracle does not clear the threshold at 120 demonstrations, the whole matrix is noise around zero and there is no result. That is retired in pre-work, not on the day: the task is chosen BECAUSE an oracle clears it at 120 demos on an SO-101 before 24 October, and if it does not clear, the task changes rather than the threshold. Second risk is code written on the day — the training grid, the IK retargeting, the trial runner and the stats all have to exist and be validated in the five weeks before, or the twelve hours go to authoring instead of data."],
    ["Hardware ask", "Larger than September's, deliberately, because the comparison IS the project and one arm cannot produce it. OpenArm full day; Wuji Hand 2 plus Wuji Glove full day as the second morphology and its teleoperation path, not a two-hour baseline; ALOHA leader arms full day to drive the OpenArm; two cameras, a tripod, an AprilTag board, a corner of a table. Not the G1 and not the Go2. An M1 bay freeing up in the evening becomes an opportunistic third row that nothing depends on."]
  ],
  files: [
    { p: "reports/astra-embodiment-transfer-2026-09-20.md", d: "The plan: the demos-to-threshold question, the three treatments, the schedule, the failure modes, the hardware ask" },
    { p: "reports/astra-hackathon-plan-2026-09-18.md", d: "The superseded certificate-desk plan, retained because its measurement rig is this project's evaluation layer" },
    { p: "reports/measurement-rig-spec-2026-09-19.md", d: "Spec for the SPRT and bootstrap that make twenty-six policies fit through one arm" },
    { p: "tools/rig/stats.py", d: "The statistical core, built and validated 2026-09-19" }
  ]
},
{
  ...
  dek: "A one-day plan for the 24 Oct Saturday Robotics × Robotics Center hackathon that runs the thesis-2 kill test in twelve hours instead of six weeks. Superseded 2026-09-20 as the submission by the embodiment-transfer plan; its measurement rig survives as that project's evaluation layer.",
  status: "superseded",
  date: "2026-09-18",
  owner: "anshu",
...
window.PROVENANCE = {
  ...
  "astra-embodiment-transfer":  { kind: "own-prior-work", note: "Written 2026-09-20 in this session, as a REPLACEMENT submission plan after you said you wanted to work on the embodiment gap and train models end to end. The event details were re-read off the Luma page (luma.com/jzbe8ytd) in a browser pane on 2026-09-20 and the hardware list is confirmed current; the page still says the date is TBD and tracks and prizes are unannounced, and registration remains approval-based. Everything else — the demos-to-threshold framing, the three treatments, the 26-run grid, the schedule and the risk ordering — is the agent's proposal. It is NOT yet reviewed by you, NOT submitted, and the load-bearing numbers (120 demonstrations per body, 10–25 minutes per training run, 8–14 trials under the SPRT) are engineering estimates that have not been measured on this hardware. The floor-effect risk in §7 is the one that would void the whole experiment and it is unretired until an oracle is run on an SO-101." },
  ...
  "eu-machinery-2027":          { kind: "own-prior-work", note: "Your regulatory research of 2026-09-07, recorded in gbrain. Regulation numbers, annex citations and dates are from the primary instruments. The 2026-09-18 additions are agent output from the daily run: the 40 and 153 notified-body counts were read from the Commission's Single Market Compliance Space register in a browser pane on that date with notification status Active, and the ISO/CD 25785-1 stage from iso.org/standard/91469.html the same way. Neither source is reachable by HTTP client. Both are point-in-time register readings, not dated publications. UPDATED 2026-09-22 by the daily run: the register was re-read the same way and now returns 41 and 152, so these are two readings of the same instrument four days apart rather than a single snapshot, and the delta is the system's own arithmetic. Intertek Deutschland NB 0905 was read from the 09-22 list. The 41-name list was scraped incompletely (37 unique names) and the count comes from the application's own result counter." },
  "assurance-seam":             { kind: "own-prior-work", note: "Your thesis, developed across the 2026-09-07 and 09-08 sessions; the crowding update comes from your own YC census. REVISED TWICE SINCE: the two kill arguments and the narrowing to non-regression are from your 2026-09-13 IRIS and robotics-market research; the embedded-evaluator counterweight is web-retrieved 2026-09-14 and is new to you. The 2026-09-17 block is agent output from the daily run: four arXiv papers retrieved that day, every quoted sentence taken from the paper's own abstract record, and none of the four independently replicated. The 2026-09-18 block is agent output from the same daily run: four arXiv papers dated 17 September, every quoted sentence taken from the paper's own abstract record, none independently replicated, and three of the four point against the thesis. The 2026-09-22 block is agent output from the same daily run: three arXiv papers dated 19 to 21 September with every quoted sentence taken from the paper's own abstract record and none independently replicated, plus one capital item whose quoted sentences come from a Business Wire release dated 17 September, which is outside that run's window and was carried deliberately with its real date." },
